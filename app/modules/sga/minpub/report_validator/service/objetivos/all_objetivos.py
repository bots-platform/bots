...
def all_objetivos(
    ...
    """
    with spark_manager.get_session() as spark:
        results = []
        
        # EXTRACT INVOQUE
        df_sga_dinamico_380 = spark.read.format("excel").load(path_sga_dinamico_380)
        df_cid_cuismp_sharepoint = spark.read.format("excel").load(path_cid_cuismp_sharepoint)
        ...
        df_word_datos_anexos_indis = (
            ...
            else spark.createDataFrame([], create_empty_schema())
        )
        ...
        # Combine results using Spark operations
        df_all = obj1_df.union(obj2_df).union(obj3_df)

        # Group and aggregate using Spark SQL
        df_grouped = df_all.groupBy("nro_incidencia").agg(
            concat_ws(" | ", collect_list("mensaje")).alias("mensaje"),
            concat_ws(" | ", collect_list("objetivo")).alias("objetivo"),
            first("TIPO REPORTE").alias("TIPO REPORTE")
        )
        ...
        df_grouped_pandas.to_excel(path_reporte_sga, index=False, engine='openpyxl')
        ...
        return df_grouped_pandas.to_dict('records')
