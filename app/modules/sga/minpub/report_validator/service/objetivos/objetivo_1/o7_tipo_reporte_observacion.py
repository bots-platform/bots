    ...
    mensaje = np.where(
        ...
        (
            np.where((df['TIPO REPORTE'] == 'RECLAMO') & (~df['reclamo_tipo_caso_valid']),
                     "RECLAMO: tipo_caso: ( " + df['tipo_caso'].astype(str) + 
            ...
            np.where((df['TIPO REPORTE'] == 'RECLAMO') & (~df['reclamo_no_monitoreo']),
                     "RECLAMO: tipo_caso: ( " + df['tipo_caso'].astype(str) + " ) no debe terminar en 'MONITOREO'.",
                    "") +

            np.where((df['TIPO REPORTE'] == 'RECLAMO') & (~df['reclamo_medidas_correctivas_valid']),
                     "RECLAMO: MEDIDAS CORRECTIVAS : ( "+ df['MEDIDAS CORRECTIVAS Y/O PREVENTIVAS TOMADAS'] +
            ...
            np.where((df['TIPO REPORTE'] == 'RECLAMO') & (~df['reclamo_observacion_valid']),
                     "RECLAMO: OBSERVACION: debe iniciar con ( " + df['observacion_pattern_componente_tipo_reporte'].astype(str) + 
                     " ) pero se obtuvo:  ( " + df['OBSERVACIÓN'].astype(str)+ " )." ,
                    "") +

            np.where((df['TIPO REPORTE'] == 'PROACTIVO') & (~df['proactivo_tipo_caso_valid']),
                     "PROACTIVO: tipo_caso : ( " + df['tipo_caso'].astype(str) + " ) debe terminar en 'MONITOREO'.",
                    "") +

            np.where((df['TIPO REPORTE'] == 'PROACTIVO') & (~df['proactivo_medidas_correctivas_valid']),
                     "PROACTIVO: MEDIDAS CORRECTIVAS: ( " + df['MEDIDAS CORRECTIVAS Y/O PREVENTIVAS TOMADAS']+ 
            ...
            np.where((df['TIPO REPORTE'] == 'PROACTIVO') & (~df['proactivo_observacion_valid']),
                    "PROACTIVO: OBSERVACION  debe ser: ( "  + df['observacion_pattern_componente_tipo_reporte'].astype(str) +
                      " )  pero se obtuvo ( "  +  df['OBSERVACIÓN'].astype(str)+ " ). " ,
                    "") 
        )
    )
